...
console.log(queryFilter.getQuery()); //get queryStr

const brandQuery = queryFilter.getQuery(); //get productList wrapper

const productsList = document.querySelector(".products-container");
const itemsNumTag = document.querySelector(".items-length"); //console.log(productsList);

const descriptionMaxLength = 150; //let proxyUrl = "https://cors-anywhere.herokuapp.com/";

//proxyUrl = ""; //开始fetch

fetch(`https://makeup-api.herokuapp.com/api/v1/products.json?${brandQuery}`).then(response => {
  console.log("response", response); // converts the response.body to a JS object
  ...
  data.forEach(el => {
    ...
    div.setAttribute("class", "product-box"); // console.log(el.description.length> descriptionMaxLength)
    ...
    if (el.description.length <= descriptionMaxLength) {
      ...
      let removeStr = el.description.slice(descriptionMaxLength); //console.log(newStr)
...
const zeroItemsP = document.querySelector(".dont-have-container>P"); // add select change listener

typeSelect.addEventListener("change", function (event) {
  productsList.innerHTML = "";
  const tagQuery = `&product_type=${event.target.value}`;
  fetch(`https://makeup-api.herokuapp.com/api/v1/products.json?${brandQuery}${tagQuery}`) //fetch(`http://makeup-api.herokuapp.com/api/v1/products.json?brand=covergirl&product_type=lipstick`)
  .then(response => {
    console.log("response", response); // converts the response.body to a JS object
    ...
    data.forEach(el => {
      ...
      console.log(el.description.length > descriptionMaxLength); //deal with description
    ...
    console.log(productsList); // console.log(productsList.innerHTML);
